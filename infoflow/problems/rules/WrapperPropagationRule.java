		...
		if (wrapperTaints != null) {
			...
			if(getManager().getConfig().isSparseOptEnabled()) {
				...
				for (Abstraction wrapperAbs : wrapperTaints) {
					...
					DataFlowNode dfg = DataFlowGraphQuery.v().useApTofindDataFlowGraph(wrapperAbs.getAccessPath(), stmt);
					if(dfg == null)
						dfg = DataFlowGraphQuery.v().useBaseTofindForwardDataFlowGraph(wrapperAbs.getAccessPath().getPlainValue(), stmt);
