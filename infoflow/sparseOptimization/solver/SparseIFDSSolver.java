 ...
 */
public class SparseIFDSSolver<N,D extends FastSolverLinkedNode<D, N>,I extends BiDiInterproceduralCFG<N, SootMethod>>
    ...
    protected static final Logger logger = LoggerFactory.getLogger(SparseIFDSSolver.class);
	 ...
	 */
	public SparseIFDSSolver(IFDSTabulationProblem<N,D,SootMethod,I> tabulationProblem) {
		...
		for (N returnSiteN : returnSiteNs) {
			...
			for(D d3: computeCallToReturnFlowFunction(callToReturnFlowFunction, d1, d2)) {
				if (memoryManager != null)
					d3 = memoryManager.handleGeneratedMemoryObject(d2, d3);
				if (d3 != null)
					propagateNormal(d1, returnSiteN, d3, n, false);
		...
		for (N m : icfg.getSuccsOf(n)) {
			...
			for (D d3 : res) {
				if (memoryManager != null && d2 != d3)
					d3 = memoryManager.handleGeneratedMemoryObject(d2, d3);
				if (d3 != null)
					propagateNormal(d1, m, d3, null, false);
	...
	protected void propagateNormal(D sourceVal, N target, D targetVal,
		/* deliberately exposed to clients */ N relatedCallSite,
		/* deliberately exposed to clients */ boolean isUnbalancedReturn) {
		propagate(sourceVal, target, targetVal, relatedCallSite, isUnbalancedReturn);
	}
	 ...
	 *        (this value is not used within this implementation but may be useful for subclasses of {@link SparseIFDSSolver})
